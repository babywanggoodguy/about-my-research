function volume_result=rad_s_volume(IT,IP,G_t,va,ta,geometry)
% 此函数为求解计算微元内的流动换热
...
Ra=CoolProp.HAPropsSI('RelHum','T',T_a_in+273.15,'P',101325,'DewPoint',Ba+273.15);%空气入口的相对湿度
Cp_a=CoolProp.HAPropsSI('cp_ha','T',T_a_in+273.15,'P',101325,'R',Ra).*0.001; %空气进口比热容
rou_a=1./CoolProp.HAPropsSI('Vha','T',T_a_in+273.15,'P',101325,'R',Ra); %空气进口密度
err_Q=1;
%% 计算主体
OT_min=IT.*0.8; % 为什么要这么设置？
OT_max=IT.*1.01;
iter=0;
while abs(err_Q) > 0.0001
    if iter > 200
        break
    else 
        iter=iter+1;
    end
    OT_mid=(OT_min+OT_max)./2;
    T_unit=(OT_mid+IT)./2;
...
%     OT_a=abs(OT_mid-IT).*Cp.*G_t./(Cp_a.*G_a)+T_a_in; %微元空气出口温度，℃
    T_a_unit=T_a_in+4; % assumed outlet air temperature
    rou_a_m=1./CoolProp.HAPropsSI('Vha','T',T_a_unit+273.15,'P',101325,'R',Ra);
    miu_a_m=CoolProp.HAPropsSI('Visc','T',T_a_unit+273.15,'P',101325,'R',Ra);
    c_a_m=CoolProp.HAPropsSI('cp_ha','T',T_a_unit+273.15,'P',101325,'R',Ra).*0.001;
    lamb_a_m=CoolProp.HAPropsSI('Conductivity','T',T_a_unit+8+273.15,'P',101325,'R',Ra);
    lambda_Al=237;
    
    %% 风侧换热系数及压降计算
    Pr_a=c_a_m.*miu_a_m./lamb_a_m.*1000;
    Re_a=v_max.*Dha.*rou_a_m./miu_a_m;
    ...
    h_a=1.*rou_a_m.*v_max.*c_a_m.*j_a.*Pr_a^(-2./3).*1000;
    ...
    Q0=abs(G_t.*Cp.*(OT_mid-IT)); % 假设微元换热量
...
OT_a=Q0./(Cp_a.*G_a)+T_a_in; %微元空气出口温度，℃
%% output parameters
volume_result(1)=Q0;
